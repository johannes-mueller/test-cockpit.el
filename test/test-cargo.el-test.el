...
(ert-deftest test-cargo-features-variable-init ()
  (let ((obj (make-instance 'test-cockpit--cargo--features-variable)))
...
(ert-deftest test-cargo-features-variable-format ()
  (let ((obj (make-instance 'test-cockpit--cargo--features-variable)))
    (mocker-let
	((test-cockpit--cargo--read-crate-features () ((:output '("foo" "bar" "baz")))))
      (oset obj value nil)
...
;; (ert-deftest test-cargo-features-variable-read ()
;;   (let ((obj (make-instance 'test-cockpit--cargo--features-variable)))
;;     (mocker-let ((test-cockpit--cargo--read-crate-features () ((:output '("foo" "bar"))))
;; 		 (completing-read (prompt features) ((:input '("feature: " ("foo" "bar")) :output "bar"))))
;;       (should (equal (transient-infix-read obj) "bar")))))

(ert-deftest test-cargo-features-variable-set ()
  (let ((obj (make-instance 'test-cockpit--cargo--features-variable)))
    (let ((test-cockpit--cargo--enabled-features nil))
      (transient-infix-set obj "foo")
      (should (equal test-cockpit--cargo--enabled-features '("foo")))
      (should (equal test-cockpit--cargo--enabled-features (oref obj value)))
      (transient-infix-set obj "bar")
      (should (equal test-cockpit--cargo--enabled-features '("foo" "bar")))
      (should (equal test-cockpit--cargo--enabled-features (oref obj value)))
      (transient-infix-set obj "bar")
      (should (equal test-cockpit--cargo--enabled-features '("foo")))
      (should (equal test-cockpit--cargo--enabled-features (oref obj value))))))
